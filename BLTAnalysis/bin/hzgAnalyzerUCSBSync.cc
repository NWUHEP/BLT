...
hzgAnalyzerUCSBSync::hzgAnalyzerUCSBSync() : BLTSelector()
{

}

hzgAnalyzerUCSBSync::~hzgAnalyzerUCSBSync()
{

}

void hzgAnalyzerUCSBSync::Begin(TTree *tree)
{
    rng = new TRandom3();
...
Bool_t hzgAnalyzerUCSBSync::Process(Long64_t entry)
{
    ...
    /*std::cout << "info about genLeptons collection: " << std::endl;
    for (unsigned int i = 0; i < genLeptons.size(); ++i) {
        std::cout << "particle " << i << " ID = " << genLeptons.at(i)->pdgId << std::endl;
    }*/

    std::string channel;
    if (genLeptons.size() > 1) {
        int genLepOneID = fabs(genLeptons.at(0)->pdgId);
        int genLepTwoID = fabs(genLeptons.at(1)->pdgId);
        //std::cout << "genLepOneID = " << genLepOneID << std::endl;
        //std::cout << "genLepTwoID = " << genLepTwoID << std::endl;
        if (genLepOneID == 11 && genLepTwoID == 11) {
            channel = "elelg";
        }
        else if (genLepOneID == 13 && genLepTwoID == 13) {
            channel = "mumug";
        }
        else {
            channel = "NONE";
            //std::cout << "EVENT CHANNEL COULD NOT BE DETERMINED!" << std::endl;
        }
    }
    else {
        channel = "NONE";
    ...
    //std::cout << "selection = " << params->selection << std::endl;

    if (params->selection != channel)
        return kTRUE;
    ...
    for (int i=0; i < jetCollection->GetEntries(); i++) {
        ...
        //std::cout << "raw pt, corr pt" << jet->ptRaw << ", " << jet->pt << std::endl;
...
void hzgAnalyzerUCSBSync::Terminate()
{
    outFile->Write();
    outFile->Close();
...
void hzgAnalyzerUCSBSync::ReportPostBegin()
{
...
void hzgAnalyzerUCSBSync::ReportPostTerminate()
{
...
{
    std::unique_ptr<hzgAnalyzerUCSBSync> selector(new hzgAnalyzerUCSBSync());
