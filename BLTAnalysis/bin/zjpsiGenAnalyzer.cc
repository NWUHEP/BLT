...
zjpsiGenAnalyzer::zjpsiGenAnalyzer() : BLTSelector()
{

}

zjpsiGenAnalyzer::~zjpsiGenAnalyzer()
{

}

void zjpsiGenAnalyzer::Begin(TTree *tree)
{
...
Bool_t zjpsiGenAnalyzer::Process(Long64_t entry)
{
    ...
    unsigned int nZMuons = 0;
    unsigned int nJpsiMuons = 0;
    vector<TLorentzVector> genMuons;
    vector<TLorentzVector> genZMuons;
    vector<TLorentzVector> genJpsiMuons;
    vector<int> zStatus;
    vector<int> jpsiStatus;
    if (!isData) {
        for (int i = 0; i < fGenParticleArr->GetEntries(); ++i) {
            ...
            //     << endl
            //
            //if (abs(particle->pdgId) == 443) 
             //   std::cout << "found a jpsi" << std::endl;

            if (
                    //(particle->status == 23 || particle->status == 1)
               ...
               ) 
            {
                ...
                copy_p4(particle, MUON_MASS, genMuonP4);
                if (mother->pdgId == 23) { 
                    //std::cout << particle->status << std::endl;
                    genZMuons.push_back(genMuonP4);
                    genMuons.push_back(genMuonP4);
                    zStatus.push_back(particle->status);
                    nZMuons++;
                    if (nZMuons > 2) {
                        for (unsigned int j = 0; j < zStatus.size(); j++) {
                            //std::cout << "Z muon status " << zStatus.at(i) << std::endl;
                        }
                    }
                }
                else if (mother->pdgId == 443) {
                    //std::cout << mother->status << std::endl;
                    genJpsiMuons.push_back(genMuonP4);
                    genMuons.push_back(genMuonP4);
                    jpsiStatus.push_back(particle->status);
                    nJpsiMuons++;
                    if (nJpsiMuons > 2) {
                        for (unsigned int j = 0; j < jpsiStatus.size(); j++) { 
                            //std::cout << "Jpsi muon status " << particle->status << std::endl;
                            //std::cout << "Jpsi status " << mother->status << std::endl;
                        }
                    }
                }
            }
    ...
    std::sort(genMuons.begin(), genMuons.end(), P4SortCondition);
    std::sort(genZMuons.begin(), genZMuons.end(), P4SortCondition);
    std::sort(genJpsiMuons.begin(), genJpsiMuons.end(), P4SortCondition);
   
    if (nZMuons != 2 || nJpsiMuons != 2) {
        //std::cout << "Not the right number of muons" << std::endl;
        return kTRUE;
    }

    TLorentzVector genZ = genZMuons[0] + genZMuons[1];
    TLorentzVector genJpsi = genJpsiMuons[0] + genJpsiMuons[1];
    
    float rapidity_bins[] = {0., 0.5, 1.0, 1.5, 2.0, 2.4};
    float pt_bins[] = {0., 5., 10., 15., 20., 25., 30., 100.};
    
    int nRapVals = sizeof(rapidity_bins)/sizeof(rapidity_bins[0]);
    int nPtVals = sizeof(pt_bins)/sizeof(pt_bins[0]);
    int rap_bin = 0;
    int pt_bin = 0;
    
    for (int i=0; i < nRapVals-1; i++) {
        if (
            fabs(genJpsi.Eta()) > rapidity_bins[i] 
            && fabs(genJpsi.Eta()) <= rapidity_bins[i+1]
           )
            {
            rap_bin = i;
            break;
    ...
    for (int i=0; i < nPtVals-1; i++) {
        if (
            fabs(genJpsi.Pt()) > pt_bins[i] 
            && fabs(genJpsi.Pt()) <= pt_bins[i+1]
           )
            {
            pt_bin = i;
            break;
    ...
    std::pair<int, int> thisBin = std::make_pair(rap_bin, pt_bin);
    ++cutMap[thisBin].at(0); 
            
    // Check generator level acceptances 
    // Should really be done with reconstructed particles
//    hTotalEvents->Fill(1);
//
//    if (fabs(genMuons[0].Eta()) < 2.4)
//        hTotalEvents->Fill(2);
//    else
//        return kTRUE;
//    
//    if (fabs(genMuons[1].Eta()) < 2.4)
//        hTotalEvents->Fill(3);
//    else
//        return kTRUE;
//    
//    if (fabs(genMuons[2].Eta()) < 2.4)
//        hTotalEvents->Fill(4);
//    else
//        return kTRUE;
//    
//    if (fabs(genMuons[3].Eta()) < 2.4)
//        hTotalEvents->Fill(5);
//    else
//        return kTRUE;
//
//    if (genMuons[0].Pt() > 22.0)
//        hTotalEvents->Fill(6);
//    else 
//        return kTRUE;
//
//    if (genMuons[1].Pt() > 10.0)
//        hTotalEvents->Fill(7);
//    else 
//        return kTRUE;
//
//    if (genMuons[2].Pt() > 4.0)
//        hTotalEvents->Fill(8);
//    else
//        return kTRUE;
//
//    if (genMuons[3].Pt() > 3.0)
//        hTotalEvents->Fill(9);
//    else 
//        return kTRUE;
//
//    return kTRUE;
    ...
    if (params->selection == "mumu") {
        ...
        // Rest of the cuts, including what is normally done in python
        if (thisZMass < 80.0 || thisZMass > 102.0)
            return kTRUE;
        
        ++cutMap[thisBin].at(9);

        if (thisJpsiMass < 2.85 || thisJpsiMass > 3.35)
            return kTRUE;

        ++cutMap[thisBin].at(10);

        //Lifetime of the jpsi
        TLorentzVector jpsiP4 = muonThreeP4 + muonFourP4;
        TVector3 dvtx = rJpsiCand - rZCand;
        TVector2 p2, r2;
        p2.Set(jpsiP4.Px(), jpsiP4.Py());
        r2.Set(dvtx.X(), dvtx.Y());
        //TVector2 *p2 = new TVector2(jpsiP4.Px(), jpsiP4.Py());
        //TVector2 *r2 = new TVector2(dvtx.X(), dvtx.Y());
        float lxy = (p2*r2)/jpsiP4.Pt();
        float txy = (100./3.)*(lxy*jpsiP4.M())/jpsiP4.Pt();
        std::cout << "txy = " << txy << std::endl;

        if (txy < -1. || txy > 5.) 
            return kTRUE;

        ++cutMap[thisBin].at(11);
...
void zjpsiGenAnalyzer::Terminate()
{
    outFile->Write();
    outFile->Close();
...
void zjpsiGenAnalyzer::ReportPostBegin()
{
    std::cout << "  ==== Begin Job =============================================" << std::endl;
    std::cout << *params << std::endl;
    std::cout << "  ============================================================" << std::endl;
    
    // Define the rapidity and pt bins
    float rapidity_bins[] = {0., 0.5, 1.0, 1.5, 2.0, 2.4};
    float pt_bins[] = {0., 5., 10., 15., 20., 25., 30., 100.};
    // Initialize the cut map
    int nRapVals = sizeof(rapidity_bins)/sizeof(rapidity_bins[0]);
    int nPtVals = sizeof(pt_bins)/sizeof(pt_bins[0]);
    int nCuts = 12;
    for (int i=0; i < nRapVals-1; i++) {
        for (int j=0; j < nPtVals-1; j++) {
            cutMap[std::make_pair(i, j)] = std::vector<int>(nCuts, 0); 
        }
    }

    // prepare csv file 
    csvOut = fopen("binned_cuts.csv", "w");
    std::stringstream ss;
    ss << "rap_bin, pt_bin, ";
    for (int i=0; i < nCuts; i++) {
        ss << "cut_" << i << ", ";
    }
    fprintf(csvOut, "%s", ss.str().c_str());
    fprintf(csvOut, "\n");
}

void zjpsiGenAnalyzer::ReportPostTerminate()
{
    ...
    //for (int i=0; i < nRapVals-1; i++) {
    //    for (int j=0; j < nPtVals-1; j++) {
    //        std::cout << cutMap[std::make_pair(i, j)].at(0) << std::endl;
    //        totevents += cutMap[std::make_pair(i, j)].at(0);
    //    }
    //}
    //std::cout << totevents << std::endl;

    std::cout << "nPtVals = " << nPtVals << std::endl;
    for (int i=0; i < nRapVals-1; i++) {
        for (int j=0; j < nPtVals-1; j++) {
            fprintf(csvOut, "%i, ", i);
            fprintf(csvOut, "%i, ", j);
            for (int k=0; k < nCuts; k++) {
                fprintf(csvOut, "%i, ", cutMap[std::make_pair(i, j)].at(k));
            }
            fprintf(csvOut, "\n");
        }
    }
...
{ 
    std::unique_ptr<zjpsiGenAnalyzer> selector(new zjpsiGenAnalyzer());
...
float zjpsiGenAnalyzer::MetKluge(float met)
{
    if (met > 500) {
        return 1.;
    }
