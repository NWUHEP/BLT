...
bool ParticleSelector::PassElectronIso(const baconhep::TElectron* el, const Cuts::elIsoCuts& cutLevel, float EAEl[7]) const 
{
    ...
    bool isoPass = false;
    if (cutLevel.cutName == "tightElIso"){
    ...
    if (cutLevel.cutName == "looseElIso"){
        if (fabs(el->scEta) <= 1.479) {
            if (combIso/el->pt < 0.0994) isoPass = true;
        } else {
            if (combIso/el->pt < 0.107) isoPass = true;
        }
    }
...
bool ParticleSelector::BTagModifier(const baconhep::TJet* jet, string tagName, string systName, float rNumber) const
{
    ...
    if (tagName == "CSVM") { 
        ...
        if (abs(jetFlavor) == 5) {
            if (systName=="up" || systName=="down" ||  systName=="central"){
          ...
          // else do nothing
            ...
            } else if (systName == "upMistag") {
                mistagSF = mistagReader->eval_auto_bounds("up", BTagEntry::FLAV_UDSG, jet->eta, jet->pt);
            } else if (systName == "downMistag") {
                mistagSF = mistagReader->eval_auto_bounds("down", BTagEntry::FLAV_UDSG, jet->eta, jet->pt);
            }
